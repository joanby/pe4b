...
Queremos saber si el consumo de combustible de tres marcas de coches $A$, $B$ y $C$ es el mismo.

Para ello, se seleccionaron 20 conductores, de los cuales, 7 fueron asignados aleatoriamente a los coches marca A, 7 a los coches marca B y los 6 restantes, a los coches marca C. 

El número de kilómetros por litro de combustible que hicieron los conductores se muestran en la tabla siguiente:
</div>
...
* $k=3$,
* $F_1=\mathrm{A}$, $F_2=\mathrm{B}$, $F_3=\mathrm{C}$.
* $n_1=n_2=7,\ n_3=6$.
...
Llamaremos a la variable $X$ `Consumo` y a la variable $F$, `Marca coche`. 

<!-- Los valores de las variables anteriores serán: -->
```{r,echo=FALSE}
conv=2.35215
Consumo=round(c(22.2,24.6,22.7,19.9,23.1,21.9,20.3,22.0,23.2,21.4,23.5,24.1,21.2,23.6,22.1,21.0,22.1,23.4,20.3,23.5)/conv,2)
Marca=c(rep(c("A","B","C"),6),"A","B")
```
...
```{r,echo=FALSE}
tabla.datos.ANOVA = data.frame(Consumo,Marca)
head(tabla.datos.ANOVA)
```

Fijémonos que hemos transformado la tabla original de 7 filas y 3 columnas en otra tabla de `r dim(tabla.datos.ANOVA)[1]` filas y `r dim(tabla.datos.ANOVA)[2]` columnas.
...
Si realizamos un gráfico del consumo según la variable marca del coche, obtenemos el gráfico siguiente:

```{r,echo=FALSE}
marca = as.factor(tabla.datos.ANOVA$Marca)
plot(marca,tabla.datos.ANOVA$Consumo,type="p",
     pch=18,cex=2,xlab="Consumo",ylab="Marca")
```
</div>
...
## Ejemplo
<div class="example">
Observamos gráficamente que las medias del consumo de las muestras correspondientes a las 3 marcas de coche parecen diferentes.

En el gráfico siguiente, dibujamos los 20 consumos correspondientes a los 20 automóviles separándolos en 3 grupos correspondientes a las tres marcas consideradas.
...
plot(1:20,tabla.datos.ANOVA$Consumo,type="p",pch=17,
     xlab="Consumo",ylab="Marca de coche",cex=0)
points(1:7,tabla.datos.ANOVA$Consumo[seq(from=1,to=20,by=3)],pch=18,cex=1.5)
points(8:14,tabla.datos.ANOVA$Consumo[seq(from=2,to=20,by=3)],pch=15,cex=1.5)
points(15:20,tabla.datos.ANOVA$Consumo[seq(from=3,to=20,by=3)],pch=17,cex=1.5)

lines(c(0,20),c(mean(tabla.datos.ANOVA$Consumo),
                mean(mean(c(tabla.datos.ANOVA$Consumo)))),lwd=3,col="red")
text(5,mean(mean(tabla.datos.ANOVA$Consumo)-0.25),"Media global",col="red")
...
* La suma total de los datos del $i$-ésimo nivel vale:
```{r,echo=FALSE}
sumas.niveles = aggregate(Consumo ~ Marca,
                          data = tabla.datos.ANOVA, FUN="sum")
sumas.niveles
```

* La media total de los datos del $i$-ésimo nivel vale:
```{r,echo=FALSE}
medias.niveles = aggregate(Consumo ~  Marca,
...
```{r,echo=FALSE}
suma.total = sum(tabla.datos.ANOVA$Consumo)
suma.total
```
* La media muestral de los datos será:
```{r,,echo=FALSE}
media.muestral = mean(tabla.datos.ANOVA$Consumo)
media.muestral
```
</div>
...
## Ejemplo
<div class="example">
Calculemos las **variabilidades** o **sumas** anteriores para los datos de nuestro ejemplo de dos maneras distintas tal como queda indicado en las expresiones.

```{r,echo=FALSE}
ni=table(tabla.datos.ANOVA$Marca)
N=sum(ni)
```
...
```{r,echo=FALSE}
(SSTotal1 =sum((tabla.datos.ANOVA$Consumo-media.muestral)^2))
```

```{r,echo=FALSE}
SSTotal = sum(tabla.datos.ANOVA$Consumo^2)-suma.total^2/N
```
...
```{r,echo=FALSE}
(SSE1=sum((tabla.datos.ANOVA$Consumo-c(rep(medias.niveles[,2],6),medias.niveles[1:2,2]))^2))
SSE=SSTotal-SSTr
```
...
```
Comprobamos que el p-valor es muy pequeño. Concluimos, por tanto, que tenemos evidencias suficientes para concluir que las medias del consumo no son iguales para las diferentes marcas de coche.
...
X=tabla.datos.ANOVA$Consumo
F=tabla.datos.ANOVA$Marca
summary(aov(X~F))
```
...
Aunque hay diferentes formas de hacerlo vamos a ver la más potente.
...
El test de Holm es un método que nos permite realizar las comparaciones entre las parejas de los distintos niveles del factor.
...
## Ejemplo 
<div class="example">
En este caso, también vemos que las pares de marcas A-B y A-C el consumo no es el mismo pero no tenemos evidencias mara rechazar que el consumo no sea el mismo en el par de marcas B-C.
...
## Ejemplo
<div class="example">
Observando los p-valores, concluimos que hay diferencias entre los coches de las marcas A-B y los coches de las marcas A-C tal como concluimos con el test de Holm.
</div>
...
Como el p-valor es muy grande, concluimos que no tenemos evidencias para rechazar la igualdad de varianzas para las 3 submuestras consideradas.
</div>
