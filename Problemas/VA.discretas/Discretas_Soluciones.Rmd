...
3. Ha llegado  al puerto  de Barcelona una partida  muy grande de latas de caviar CAVIARFRIO de 1KG.  El transportista sabe que la partida  contiene un 10\% de latas con defectos visibles en el exterior (golpes, deformaciones, arañazos). Supongamos que encargamos $n=10$ latas  que se escogen al azar de esta partida. Sea $X$ el número de latas defectuosas entre las $n$ latas.
...
La variable $X$ sigue una distribución $B(n=3,p=0.01)$ es una binomial con probabilidad de ·éxito· $P(\mbox{lata defectuosa})=0.01.$
...
\renewcommand*{\arraystretch}{0.5}
P_X(x)= P(X=x)&=&
\left\{\begin{array}{ll}{n\choose x}\cdot  p^x\cdot  (1-p)^{n-x} & \mbox{ si } x=0,1,\ldots,n\\  0  & \mbox{ en otro caso.}\end{array}\right.\\
&=&\left\{\begin{array}{ll}{n\choose x}\cdot  0.15^x\cdot  (1-0.15)^{n-x} & \mbox{ si } x=0,1,\ldots,n\\  0  & \mbox{ en otro caso.}\end{array}\right.
\end{eqnarray*}
...
$E(X)=\lambda=13$ y $Var(X)=\frac{1}{\lambda}=\frac{1}{13}=`r round(1/13,4)`.$
