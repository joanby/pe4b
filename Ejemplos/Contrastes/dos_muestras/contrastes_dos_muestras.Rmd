...
\red{Desconocemos las dos desviaciones típicas poblacionales} $\sigma_1$ y $\sigma_2$ de cada una de las valoraciones de la campaña.
</div>
...
$$
f=\frac{ \left( \frac{`r s1`^2}{`r n1`}+\frac{`r s1`^2}{`r n2`}\right)^2}
{\frac1{`r n1-1`}\left(\frac{`r s1`^2}{`r n1`}\right)^2+\frac1{`r n2-1`}\left(\frac{`r s2`^2}{`r n2`}\right)^2}=`r f`
.
$$
...
El **Estadístico de contraste** para el contraste anterior es: $F=\frac{\widetilde{S}_1^2}{\widetilde{S}_2^2}\sim F_{24,35}.$
...
## Ejemplo  contraste de  medias emparejadas, comparación de dos algoritmos de fidelización.
<div class="example-sol">
La media y la desviación típica muestrales de las diferencias son $\overline{d}=`r mean(diferencias)`,$ $\tilde s_d = `r round(sd(diferencias),3)`.$
