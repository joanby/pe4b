...
$$z_0= \frac{\overline{x}-`r mu0` }{\frac{\sigma}{\sqrt{n}}}= \frac{`r m`-`r mu0`}{\frac{`r sigma`}{\sqrt{`r n ` }}}=\frac{`r m-mu0`}{\frac{`r sigma`}{`r sqrt(n)`}}=`r z0`>z_{0.05}=`r -z0.05`.$$


Como $z_0=`r round(15/9,4)` > 1.645$  no podemos  aceptar  que la producción  no haya superado la media de $90$ unidades.
...
Por lo tanto debemos concluir que \blue{la nueva organización puede que no  mejore el proceso de producción}. Si los trabajadores están más satisfechos producen un poco más, \red{aunque notemos que la media a aumentado en $0.5$ unidades} esto puede deberse  la aleatoriedad de toda prueba.
...
s_X&=&\sqrt{\frac{\sum x^2}{n}-\overline{x}^2}=
    \sqrt{\frac{`r sumax2`}{`r n`}-`r m`^2}\\
    &=&
\sqrt{`r sumax2/n- m^2`}=`r round(sumax2/n- m^2,4)`.
\end{eqnarray*}
...
\blue{El estadístico de contraste es al nivel de significación $\alpha=0.05$  e: Rechazar $H_0$ si }

$$
t_0= \frac{\overline{x}-`r mu0` }{\frac{\sigma}{\sqrt{n}}}= \frac{`r m`-`r mu0`}{\frac{`r sigma`}{\sqrt{`r n ` }}}=\frac{`r m-mu0`}{\frac{`r sigma`}{`r sqrt(n)`}}=`r t0`>t_{0.05,n-1}=`r - t0.05`.
$$


Como $t_0=`r t0` > `r t0.05`$  no podemos  aceptar  que la producción  no haya superado la media de $90$ unidades.
...
$$
\left(\overline{x}+t_{\alpha,n-1}\frac{\sigma}{\sqrt{n}},+\infty\right)
$$
